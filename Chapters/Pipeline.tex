...
the owner variable is not relevant for the pipeline. 'mail' is used to send SLURM updates from the HPC. if the pipeline is run as is, then it requires a SLURM server. information on start, fail, end and time limits are sent to the mail. \\
The subprocesses of preprocessing the images, segmenting on them and then evaluating with a given section are only started if their respective value is set to 'true'. \\
...
\end{lstlisting}
all paths above are required to be defined before starting the pipeline. 'home' defines the directory where the output - images \& transcripts, segments, evaluation, logs - will be saved and where the ground-truth as well as the file to it's coordinates are located. 'data\us path' is the directory containing the Xenium output as defined above. 'sample\us name' the name to give the sample run, example: TMA1 for tissue microarray 1. 'gt\us path' the path to the groundtruth masks. required if evaluation based on Jaccard and co is used. 'sections\us path' the path to a json file of the section to evaluate on. required if running any evaluation. the provided ground truth must be of the same dimensions as the sections\us path. \\
...
All tasks are send to the HPC if the pipeline is run. the parameters below configure for the SLURM nodes. using the function \textit{XenSegEval.utils.submit\us sbatch} the parameteres are collected and written into a job file in \textit{'(home)/\textless sample\us name\textgreater/run/jobs/\textless method\textgreater.sh'}
\begin{lstlisting}[caption=config, style=customIni]
[sbatch]
...
Below an example bash file for the evaluation of {\color{dinocell}dinocell} which is then send to an HPC node and executed. 
...
The parameters of the segmentation are configured for each algorithm individually. to adjust which algorithms are run, exclude their parameters and the table name \textit{[methods.\textless method\textgreater]}. \\
